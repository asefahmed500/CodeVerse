...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (action === 'duplicate') {
            ...
            const duplicateRecursively = async (node: any, newParentId: mongoose.Types.ObjectId | null, visited: Set<string>): Promise<any> => {
                if (visited.has(node._id.toString())) {
                    // This prevents infinite loops in case of data corruption, though schema should prevent this.
                    return null;
                }
                visited.add(node._id.toString());
                
                const newId = new mongoose.Types.ObjectId();
                const siblingNames = (await File.find({ parentId: newParentId, userId }, 'name').lean()).map(f => f.name);
                ...
                if (node.isFolder) {
                    ...
                    for (const child of children) {
                        const newChild = await duplicateRecursively(child, newNode._id, visited);
                        if (newChild) {
            ...
            const duplicated = await duplicateRecursively(original, original.parentId, new Set());
